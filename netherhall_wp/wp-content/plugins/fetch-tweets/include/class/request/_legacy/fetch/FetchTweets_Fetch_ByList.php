...
abstract class FetchTweets_Fetch_ByList extends FetchTweets_Fetch_BySearch {
    ...
    public function getListNamesFromScreenName( $sScreenName, $iAccountID=0 ) {
        ...
        $_aListIDs = array();
        foreach( $this->getListsByScreenName( $sScreenName, $iAccountID ) as $aListDetail ) {
            $_aListIDs[ $aListDetail['id'] ] = $aListDetail['name'];
        }
        return $_aListIDs;
    ...
    public function getListsByScreenName( $sScreenName, $iAccountID=0, $iCacheDuration=600 ) {
        ...
        // Construct a request URI e.g. https://api.twitter.com/1.1/lists/list.json?screen_name=twitterapi
        $_sRequestURI = add_query_arg(
            array(
                'screen_name'   =>  $sScreenName,
                
                // The followings are for the plugin transient ID, NOT Twitter API parameters.
