 ...
 *      'field_id'      => 'date_time',
 *      'type'          => 'date_time',
 *      'title'         => __( 'Date & Time', 'fetch-tweets-field-type-pack' ),
 ...
 *      'type'          => 'date_time',
 *      'title'         => __( 'Multiple Date and Time', 'fetch-tweets-field-type-pack' ),
 *      'label'         => __( 'Default', 'fetch-tweets-field-type-pack' ), 
 *      'time_format'   => 'H:mm',
 *      'date_format'   => 'yy-mm-dd', // yy/mm/dd is the default format.
 *      'delimiter'     => '<br />',     
 *      'attributes'    => array(
 ...
 *      ),     
 *      array(
 *          'label'         => __( 'AM PM', 'fetch-tweets-field-type-pack' ), 
 *          'time_format'   => 'hh:mm tt',
 *      ),
 *      array(
 *          'label'         => __( 'Time Zone', 'fetch-tweets-field-type-pack' ), 
 *          'time_format'   => 'hh:mm tt z',
 *      ),
 *      array(
 *          'label'         => __( 'Number Of Months', 'fetch-tweets-field-type-pack' ), 
 *          'options'       => array(
 *              'numberOfMonths' => 3,
 *          ),
 *      ),     
 *      array(
 *          'label'         => __( 'Min & Max Dates', 'fetch-tweets-field-type-pack' ), 
 *          'options'       => array(
 *              'numberOfMonths'    => 2,
 *              'minDate'           => 0,
 *              'maxDate'           => 30,
 ...
 *      'type'          => 'date_time',
 *      'title'         => __( 'Repeatable Date & Time Fields', 'fetch-tweets-field-type-pack' ),
 *      'repeatable'    => true,
 *      'options'       => array(
 *          'timeFormat'    => 'HH:mm:ss',
 *          'stepHour'      => 2,
 ...
 *      'type'          => 'date_time',
 *      'title'         => __( 'Sortable', 'fetch-tweets-field-type-pack' ),
 *      'sortable'      => true,
 *      'attributes'    => array(
 *          'size' => 30,
 *      ),
 ...
 *              array(
 *                  'value' => -300,
 *                  'label' => __( 'Eastern', 'fetch-tweets-field-type-pack' ),
 *              ),
 *              array(
 *                  'value' => -360,
 *                  'label' => __( 'Central', 'fetch-tweets-field-type-pack' ),
 *              ),     
 *              array(
 *                  'value' => -420,
 *                  'label' => __( 'Mountain', 'fetch-tweets-field-type-pack' ),
 *              ),     
 *              array(
 *                  'value' => -480,
 *                  'label' => __( 'Pacific', 'fetch-tweets-field-type-pack' ),
 *              ),     
 *          ),
 *      ),
 ...
 * @version     1.0.0
 */
class DateTimeCustomFieldType extends FetchTweets_AdminPageFramework_FieldType {
...
jQuery( document ).ready( function(){
    jQuery().registerFetchTweets_AdminPageFrameworkCallbacks( {  
        /**
        ...
        repeated_field: function( oCloned, aModel ) {
            ...
            var sOptionID = jQuery( oCloned ).closest( '.fetch-tweets-sections' ).attr( 'id' ) 
                + '_' 
                + jQuery( oCloned ).closest( '.fetch-tweets-fields' ).attr( 'id' );    // sections id + _ + fields id 
            ...
            . "<div class='fetch-tweets-input-label-container'>"
                . "<label for='{$aField['input_id']}'>"
                    ...
                    . ( $aField['label'] && ! $aField['repeatable']
                        ? "<span class='fetch-tweets-input-label-string' style='min-width:" . $this->getLengthSanitized( $aField['label_min_width'] ) . ";'>" . $aField['label'] . "</span>"
                        : "" 
                    )
            ...
            return 
                "<script type='text/javascript' class='date-time-picker-enabler-script'>
                    jQuery( document ).ready( function() {
                        ...
                        var sOptionID = jQuery( '#{$sInputID}' ).closest( '.fetch-tweets-sections' ).attr( 'id' ) + '_' + jQuery( '#{$sInputID}' ).closest( '.fetch-tweets-fields' ).attr( 'id' );
