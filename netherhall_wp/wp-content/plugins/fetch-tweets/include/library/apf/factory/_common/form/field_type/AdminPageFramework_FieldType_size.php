...
class FetchTweets_AdminPageFramework_FieldType_select extends FetchTweets_AdminPageFramework_FieldType {
    ...
    protected function getStyles() {
        return ".fetch-tweets-field-select .fetch-tweets-input-label-container {vertical-align: top; }.fetch-tweets-field-select .fetch-tweets-input-label-container {padding-right: 1em;}";
    }
    protected function getField($aField) {
        $_oSelectInput = new FetchTweets_AdminPageFramework_Input_select($aField['attributes']);
        if ($aField['is_multiple']) {
            $_oSelectInput->setAttribute(array('select', 'multiple'), 'multiple');
        }
        return $aField['before_label'] . "<div " . $this->getLabelContainerAttributes($aField, 'fetch-tweets-input-label-container fetch-tweets-select-label') . ">" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . $_oSelectInput->get($aField['label']) . $aField['after_input'] . "<div class='repeatable-field-buttons'></div>" . "</label>" . "</div>" . $aField['after_label'];
    }
}
class FetchTweets_AdminPageFramework_FieldType_size extends FetchTweets_AdminPageFramework_FieldType_select {
    ...
    protected function getStyles() {
        return ".fetch-tweets-field-size input {text-align: right;}.fetch-tweets-field-size select.size-field-select {vertical-align: 0px; }.fetch-tweets-field-size label {width: auto; } .form-table td fieldset .fetch-tweets-field-size label {display: inline;}";
    }
    protected function getField($aField) {
    ...
    private function _getNumberInputPart(array $aField, array $aBaseAttributes, $isKey, $bMultiLabels) {
        ...
        $_sLabel = $this->getElementByLabel($aField['label'], $isKey, $aField['label']);
        return "<label " . $this->getAttributes($_aSizeLabelAttributes) . ">" . $this->getElement($aField, $bMultiLabels ? array('before_label', $isKey, 'size') : array('before_label', 'size')) . ($aField['label'] && !$aField['repeatable'] ? "<span " . $this->getLabelContainerAttributes($aField, 'fetch-tweets-input-label-string') . ">" . $_sLabel . "</span>" : "") . "<input " . $this->getAttributes($_aSizeAttributes) . " />" . $this->getElement($aField, $bMultiLabels ? array('after_input', $isKey, 'size') : array('after_input', 'size')) . "</label>";
    }
    private function _getUnitSelectInput(array $aField, array $aBaseAttributes, $isKey, $bMultiLabels) {
        ...
        $_oUnitInput = new FetchTweets_AdminPageFramework_Input_select($_aUnitAttributes + array('select' => $_aUnitAttributes));
