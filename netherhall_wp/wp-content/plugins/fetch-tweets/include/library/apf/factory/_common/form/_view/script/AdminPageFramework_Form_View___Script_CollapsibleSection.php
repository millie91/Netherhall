...
class FetchTweets_AdminPageFramework_Form_View___Script_CollapsibleSection extends FetchTweets_AdminPageFramework_Form_View___Script_Base {
...
( function( $ ) {
    ...
    jQuery( document ).ready( function() {
        jQuery( this ).initializeFetchTweets_AdminPageFrameworkCollapsibleSections();
    });              
     ...
     * @since    3.7.0
     */
    jQuery( document ).bind( 'fetch-tweets_saved_widget', function( event, oWidget ){
        jQuery( oWidget ).initializeFetchTweets_AdminPageFrameworkCollapsibleSections();
    });    
    
    $.fn.initializeFetchTweets_AdminPageFrameworkCollapsibleSections = function() {
        
        // Expand collapsible sections that are set not to collapse by default 
        jQuery( this ).find( '.fetch-tweets-collapsible-sections-title[data-is_collapsed=\"0\"]' )
            .next( '.fetch-tweets-collapsible-sections-content' )
            .slideDown( 'fast' );
        jQuery( this ).find( '.fetch-tweets-collapsible-section-title[data-is_collapsed=\"0\"]' )
            .closest( '.fetch-tweets-section-table' )
            .find( 'tbody' )
            .slideDown( 'fast' );
            
        // Hide collapsible sections of 'section' containers as they are somehow do not get collapsed by default.
        jQuery( this ).find( '.fetch-tweets-collapsible-section-title[data-is_collapsed=\"1\"]' )
            .closest( '.fetch-tweets-section-table' )
            .find( 'tbody' )
            .hide();
        
        // Bind the click event to the title element.
        jQuery( this ).find( '.fetch-tweets-collapsible-sections-title, .fetch-tweets-collapsible-section-title' ).enableFetchTweets_AdminPageFrameworkCollapsibleButton();
        
        // Insert the toggle all button.
        jQuery( this ).find( '.fetch-tweets-collapsible-title[data-toggle_all_button!=\"0\"]' ).each( function(){
            
            var _oThis        = jQuery( this ); // to access from inside the below each() method.
            var _bForSections = jQuery( this ).hasClass( 'fetch-tweets-collapsible-sections-title' );   // or for the 'section' container.
            ...
            jQuery.each( _aPositions, function( iIndex, _sPosition ) {
                ...
                var _sLeftOrRight = -1 !== jQuery.inArray( _sPosition, [ 'top-right', 'bottom-right', '0' ] )   // if found
                    ? 'right'   // default
                    : 'left';            
                _oButton.find( '.fetch-tweets-collapsible-toggle-all-button' ).css( 'float', _sLeftOrRight );
                ...
                if ( _bForSections ) {
                    var _oTargetElement = 'before' === _sTopOrBottom
                        ? _oThis
                        : _oThis.next( '.fetch-tweets-collapsible-content' );
                        _oTargetElement[ _sTopOrBottom ]( _oButton );
                } else {    // for 'section' containers
                    _oThis.closest( '.fetch-tweets-section' )[ _sTopOrBottom ]( _oButton );
                }                
                ...
                _oButton.click( function(){                  
                    ...
                    var _oButtons = _bForSections
                        ? jQuery( this ).closest( '.fetch-tweets-sectionset' ).siblings().andSelf().find( '> .fetch-tweets-collapsible-toggle-all-button-container' )
                        : jQuery( this ).siblings( '.fetch-tweets-collapsible-toggle-all-button-container' ).andSelf();
                    _oButtons.toggleClass( 'flipped' );
                    if ( _bForSections ) {
                        _oButton.parent().parent().children().children( '* > .fetch-tweets-collapsible-title' ).each( function() {
                            jQuery( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    } else {
                        _oButton.closest( '.fetch-tweets-sections' ).children( '.fetch-tweets-section' ).children( '.fetch-tweets-section-table' ).children( 'caption' ).children( '.fetch-tweets-collapsible-title' ).each( function() {
                            jQuery( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    }
     ...
     */
    $.fn.enableFetchTweets_AdminPageFrameworkCollapsibleButton = function() {
        ...
        jQuery( this ).click( function( event, sContext ){
            ...
            var _sContainerType = jQuery( this ).hasClass( 'fetch-tweets-collapsible-sections-title' )
                ? 'sections'
                : 'section';
            var _oTargetContent = 'sections' === _sContainerType
                ? jQuery( this ).next( '.fetch-tweets-collapsible-content' ).first()
            ...
            if ( 'expand' === _sAction && _oThis.data( 'collapse_others_on_expand' ) ) {
                _oThis.parent().parent().children().children( '* > .fetch-tweets-collapsible-content' ).not( _oTargetContent ).slideUp( 'fast', function() {
                    jQuery( this ).prev( '.fetch-tweets-collapsible-title' ).addClass( 'collapsed' );
                });
            }
...
JAVASCRIPTS;
    ...
    static private function _getToggleAllButtonHTML($oMsg) {
        ...
        return "<div class='fetch-tweets-collapsible-toggle-all-button-container'>" . "<span class='fetch-tweets-collapsible-toggle-all-button button " . $_sDashIconSort . "'" . " title='" . esc_attr($_sLabelToggleAllSections) . "'>" . $_sText . "</span>" . "</div>";
    }
}
