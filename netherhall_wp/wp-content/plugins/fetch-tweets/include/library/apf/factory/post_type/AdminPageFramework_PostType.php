...
abstract class FetchTweets_AdminPageFramework_PostType_Router extends FetchTweets_AdminPageFramework_Factory {
...
abstract class FetchTweets_AdminPageFramework_PostType_Model extends FetchTweets_AdminPageFramework_PostType_Router {
    public function __construct($oProp) {
        parent::__construct($oProp);
        ...
        if ($this->oProp->bIsAdmin) {
            ...
            if ($this->oProp->sCallerPath) {
                new FetchTweets_AdminPageFramework_PostType_Model__FlushRewriteRules($this);
            }
        }
    }
    ...
    public function _replyToRegisterPostType() {
        ...
        new FetchTweets_AdminPageFramework_PostType_Model__SubMenuOrder($this);
    }
...
abstract class FetchTweets_AdminPageFramework_PostType_View extends FetchTweets_AdminPageFramework_PostType_Model {
    public function __construct($oProp) {
        parent::__construct($oProp);
        if ($this->oProp->bIsAdmin) {
    ...
    public function _replyToPrintStyle() {
        ...
        if (!empty($this->oProp->sStyle)) {
            echo "<style type='text/css' id='fetch-tweets-style-post-type'>" . $this->oProp->sStyle . "</style>";
        }
    }
...
abstract class FetchTweets_AdminPageFramework_PostType extends FetchTweets_AdminPageFramework_PostType_Controller {
    protected $_sStructureType = 'post_type';
    public function __construct($sPostType, $aArguments = array(), $sCallerPath = null, $sTextDomain = 'fetch-tweets') {
        if (empty($sPostType)) {
            return;
        }
        ...
        $this->oProp->sPostType = FetchTweets_AdminPageFramework_WPUtility::sanitizeSlug($sPostType);
        $this->oProp->aPostTypeArgs = $aArguments;
    ...
    private function _getCallerScriptPath($sCallerPath) {
        ...
        $_sPageNow = FetchTweets_AdminPageFramework_Utility::getElement($GLOBALS, 'pagenow');
        if (in_array($_sPageNow, array('edit.php', 'post.php', 'post-new.php', 'plugins.php', 'tags.php', 'edit-tags.php', 'term.php'))) {
            return FetchTweets_AdminPageFramework_Utility::getCallerScriptPath(__FILE__);
        }
        return null;
    }
}
