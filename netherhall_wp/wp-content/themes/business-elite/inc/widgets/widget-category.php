...
class Business_elite_categ extends WP_Widget
{
	function __construct()
	{
		...
		parent::__construct(false, $name = __('Categories Posts', "business-elite"), $widget_ops, $control_ops);
	}
	...
	function update($new_instance, $old_instance)
	{
		...
		$instance['categ_id'] = sanitize_text_field($new_instance['categ_id']);
	...
	function form($instance)
	{
		...
		<p>
			<label for="<?php echo $this->get_field_id('title'); ?>">Title:</label><input class="widefat"
		...
		<p>
			<label for="<?php echo $this->get_field_id('categ_id'); ?>"><?php _e("Select Category:", "business-elite"); ?></label>
			<select name="<?php echo $this->get_field_name('categ_id'); ?>" id="<?php echo $this->get_field_id('categ_id') ?>"
				...
				<option value="0"><?php _e("Select Category:", "business-elite"); ?></option>
				<?php
		...
		<p>
			<label for="<?php echo $this->get_field_id('post_count'); ?>"><?php _e("Number of Posts:", "business-elite"); ?></label><input
				id="<?php echo $this->get_field_id('post_count'); ?>" name="<?php echo $this->get_field_name('post_count'); ?>"
				type="text" value="<?php echo $post_count; ?>" size="6"/>
		</p>
		<?php
	}
}
